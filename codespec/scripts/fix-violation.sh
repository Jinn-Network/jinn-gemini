...
if ../../scripts/conductor-setup.sh > /tmp/conductor-setup-${VIOLATION_ID}.log 2>&1; then
...
if claude -p "/fix-violation $VIOLATION_ID" \
...
---
🤖 Generated by [\`fix-violation.sh\`](../../codespec/scripts/fix-violation.sh)" 2>&1 | grep -oP 'https://github.com/.*/pull/\d+' | head -1)
