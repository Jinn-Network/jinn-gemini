...
- Ventures registry (CRUD operations)
- Services registry (CRUD operations)

## Ventures Registry Tools

When working with ventures (creating, querying, updating, or deleting), use these tools:
...
```json
{
  "name": "My Venture",
  "ownerAddress": "0x1234...",
  "blueprint": "{\"invariants\":[{\"id\":\"INV-001\",\"description\":\"Test\"}]}"
}
```

**Query ventures:**
- Get by ID: `{ "mode": "get", "id": "<uuid>" }`
- Get by slug: `{ "mode": "by_slug", "slug": "my-venture" }`
- List active: `{ "mode": "list", "status": "active", "limit": 20 }`
...
For detailed documentation, see `gemini-agent/mcp/tools/docs/ventures.md`.
