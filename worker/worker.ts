...
const jobIdFlagIndex = process.argv.findIndex(
  (arg) => arg === "--job-id" || arg === "-j",
);
const targetJobId =
  jobIdFlagIndex !== -1 ? process.argv[jobIdFlagIndex + 1] : null;
const stopOnChief = process.argv.includes("--stop-on-chief");
const singleJobMode =
  process.argv.includes("--single-job") || Boolean(targetJobId);
...
// Validate required environment variables for service role access
const SUPABASE_SERVICE_ROLE_KEY = getRequiredString(
  ...
  try {
    const env = {
      ...
      PLAYWRIGHT_PROFILE_DIR:
        ...
        `${process.cwd()}/.playwright-mcp/google-profile`,
    ...
    const { stdout } = await asyncExecFile(
      "npx",
      ["tsx", "scripts/civitai-read-buzz.ts"],
      {
        env,
        timeout: 60_000,
      },
...
async function readLastBuzzSnapshot(): Promise<{
  current?: string;
  previous?: string;
} | null> {
  try {
...
async function resolveActiveJobDefinitionId(
  jobName: string,
): Promise<string | null> {
  try {
    const res = await readRecords({
      table_name: "jobs",
      filter: { name: jobName, is_active: true },
    });
    const parsed = parseToolResponse(res);
    if (!parsed.success) return null;
    const rows = Array.isArray(parsed.data)
...
function isRetryableError(error: any): boolean {
  ...
  workerLogger.debug(
    `Checking if error is retryable: "${combined.substring(0, 200)}..."`,
  );
...
function calculateRetryDelayWithJitter(retryCount: number): number {
  ...
  workerLogger.debug(
    `Backoff calculation: base=${base}ms, max=${noJitter}ms, chosen (jitter)=${jitteredDelay}ms`,
  );
  ...
  try {
    const report = {
      ...
      final_output:
        ...
        String(context.result.output).trim().length > 0
          ? context.result.output
      ...
      error_message:
        ...
        context.result?.telemetry?.errorMessage ||
      ...
      error_type: context.error
        ? categorizeWorkerError(context.error)
    ...
    try {
      ...
      const jobDefinitionId =
        (context.job as any).job_definition_id ||
        context.job.parent_job_definition_id ||
        "";
      process.env.JINN_JOB_DEFINITION_ID = jobDefinitionId || "";
      process.env.JINN_JOB_NAME = context.job.job_name || "";
      process.env.JINN_PROJECT_RUN_ID = (context.job.project_run_id ??
        "") as string;
      process.env.JINN_SOURCE_EVENT_ID = (context.job.source_event_id ??
        "") as string;
      process.env.JINN_PROJECT_DEFINITION_ID = (context.job
        .project_definition_id ?? "") as string;
      ...
      if (!parsed.success) {
        ...
        workerLogger.info(
          `Job report stored successfully for ${context.job.id}${newId ? ` (report_id=${newId})` : ""}`,
        );
      ...
      if (prevEnv.JINN_JOB_ID !== undefined)
        process.env.JINN_JOB_ID = prevEnv.JINN_JOB_ID;
      else delete process.env.JINN_JOB_ID;
      if (prevEnv.JINN_JOB_DEFINITION_ID !== undefined)
        process.env.JINN_JOB_DEFINITION_ID = prevEnv.JINN_JOB_DEFINITION_ID;
      else delete process.env.JINN_JOB_DEFINITION_ID;
      if (prevEnv.JINN_JOB_NAME !== undefined)
        process.env.JINN_JOB_NAME = prevEnv.JINN_JOB_NAME;
      else delete process.env.JINN_JOB_NAME;
      if (prevEnv.JINN_PROJECT_RUN_ID !== undefined)
        process.env.JINN_PROJECT_RUN_ID = prevEnv.JINN_PROJECT_RUN_ID;
      else delete process.env.JINN_PROJECT_RUN_ID;
      if (prevEnv.JINN_SOURCE_EVENT_ID !== undefined)
        process.env.JINN_SOURCE_EVENT_ID = prevEnv.JINN_SOURCE_EVENT_ID;
      else delete process.env.JINN_SOURCE_EVENT_ID;
      if (prevEnv.JINN_PROJECT_DEFINITION_ID !== undefined)
        process.env.JINN_PROJECT_DEFINITION_ID =
          prevEnv.JINN_PROJECT_DEFINITION_ID;
...
function categorizeWorkerError(error: any): string {
  ...
  if (message.includes("Gemini process exited with code"))
...
function shouldWaitForRateLimit(): {
  ...
  const quotaCooldownRemaining =
    quotaErrorTime > 0
      ? RATE_LIMIT.cooldownAfterQuotaError - (now - quotaErrorTime)
      : 0;
  const internalCooldownRemaining =
    internalErrorCooldownTime > 0
      ? INTERNAL_ERROR_COOLDOWN_MS - (now - internalErrorCooldownTime)
      : 0;
  ...
  if (quotaCooldownRemaining > 0 || internalCooldownRemaining > 0) {
    const maxCooldown = Math.max(
      ...
      internalCooldownRemaining,
    );
    const reason =
      quotaCooldownRemaining > internalCooldownRemaining
    ...
    if (parsed.meta && typeof parsed.meta.ok === "boolean") {
      if (parsed.meta.ok === false) {
        return {
          ...
          data: parsed.data,
          error:
            parsed.meta.message || parsed.meta.code || "Tool reported failure",
        };
      }
...
function extractErrorContext(error: any): string | null {
  ...
  if (
    ...
    stderr.includes("path validation failed")
  ) {
...
- '../worker/worker.ts'`;
  ...
  if (
    message.includes("resolves outside the allowed workspace directories") ||
    stderr.includes("resolves outside the allowed workspace directories")
  ) {
...
Use relative paths and ensure all file operations stay within the project boundaries.`;
  ...
  if (
    ...
    stderr.includes("Invalid arguments for tool")
  ) {
...
- Wrong data types for parameters`;
  ...
  if (
    ...
    stderr.includes("Invalid enum value")
  ) {
...
use one of those exact values, not 'search_events' or similar.`;
  ...
  if (
    ...
    stderr.includes("MCP error -32602")
  ) {
...
function stringifyForPrompt(
  obj: any,
  options?: { maxChars?: number; largeFieldTruncate?: number },
): string {
      ...
      if (input && typeof input === "object") {
        ...
        for (const [k, v] of Object.entries(input)) {
          if (k === "content" || k === "output") {
            if (typeof v === "string")
              out[k] = truncateString(v, largeFieldTruncate);
            else out[k] = v;
          } else {
...
function truncateContext(
  ...
  if (isDelegatedWork && context.child_jobs) {
    ...
    const truncatedJobs = context.child_jobs.map((job: any) => {
      return {
        ...
        output: truncateString(
          job.output,
          TOKEN_CONFIG.JOB_OUTPUT_TRUNCATE_CHARS,
        ),
        artifacts: job.artifacts?.map((art: any) => ({
          ...art,
          content: truncateString(
            art.content,
            TOKEN_CONFIG.ARTIFACT_CONTENT_TRUNCATE_CHARS,
          ),
        })),
      };
    });
    ...
    if (source.output) {
      truncatedSource.output = truncateString(
        source.output,
        TOKEN_CONFIG.JOB_OUTPUT_TRUNCATE_CHARS,
      );
    }
    if (source.related_artifacts) {
      truncatedSource.related_artifacts = source.related_artifacts.map(
        (art: any) => ({
          ...art,
          content: truncateString(
            art.content,
...
async function processPendingJobs(): Promise<boolean> {
  workerLogger.info(
    `Worker ${workerId} starting up, checking for ${targetJobId ? `targeted job ${targetJobId}` : "pending jobs"}...`,
  );
  if (debugMode) {
    workerLogger.debug(
      "Worker running in debug mode - Gemini CLI will use --debug flag",
    );
  }
  ...
  if (rateLimitCheck.shouldWait) {
    ...
    await new Promise((resolve) =>
      setTimeout(resolve, rateLimitCheck.waitTime),
    );
  }
  ...
  try {
    ...
    workerLogger.error(
      { error: parseErr },
      "Failed to parse jobs from database",
    );
    return false;
  }
  ...
  if (!jobs || jobs.length === 0) {
    workerLogger.info(
      targetJobId
        ? `No job found with id ${targetJobId}`
  ...
  workerLogger.info(
    targetJobId
      ? `Found targeted job ${jobs[0].id} (status=${jobs[0].status || "UNKNOWN"})`
  ...
  if (stopOnChief && job.job_name === "Chief Orchestrator") {
    workerLogger.info(
      "`--stop-on-chief` flag set; detected Chief Orchestrator job. Exiting without claiming or processing.",
    );
    process.exit(0);
  }

  workerLogger.info(
    { jobId: job.id, jobName: job.job_name, forced: !!targetJobId },
  ...
  while (retryCount <= RETRY_CONFIG.maxRetries) {
    ...
    const composeTriggerContextSection = (triggerContext?: any): string => {
      ...
      const json = stringifyForPrompt(triggerContext, {
        maxChars: 10000,
        largeFieldTruncate: 400,
      });
      return `\n\n### Trigger Context\n\n${json}`;
    };

    const composeDelegatedWorkContextSection = (
      delegatedWorkContext?: any,
    ): string => {
      if (!delegatedWorkContext) return "";
      const json = stringifyForPrompt(delegatedWorkContext, {
        maxChars: 14000,
        largeFieldTruncate: 400,
      });
    ...
    const composeRecentRunsContextSection = (
      recentRunsContext?: any,
    ): string => {
      if (!recentRunsContext) return "";
      const json = stringifyForPrompt(recentRunsContext, {
        maxChars: 8000,
        largeFieldTruncate: 300,
      });
      return `\n\n### Recent Runs Context\n\n${json}`;
    };
    ...
    const triggerContextSection = composeTriggerContextSection(
      truncateContext(
        job.trigger_context,
        TOKEN_CONFIG.TRIGGER_CONTEXT_MAX_TOKENS,
        false,
      ),
    );
    const delegatedWorkContextSection = composeDelegatedWorkContextSection(
      truncateContext(
        job.delegated_work_context,
        TOKEN_CONFIG.DELEGATED_WORK_CONTEXT_MAX_TOKENS,
        true,
      ),
    );
    const recentRunsContextSection = composeRecentRunsContextSection(
      job.recent_runs_context,
    );
    const rawPrompt =
      `${jobHeader}${job.input || ""}${inboxSection}${triggerContextSection}${delegatedWorkContextSection}${recentRunsContextSection}`.trim();
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          `Refusing to force-run job ${job.id} because it is already IN_PROGRESS by worker ${job.worker_id}`,
        );
      }
      ...
      const claimResult = await updateRecords({
        ...
        filter: targetJobId
          ? { id: job.id }
          : { id: job.id, status: "PENDING" }, // When targeted, bypass status predicate
        updates: {
          status: "IN_PROGRESS",
          worker_id: workerId,
        },
      ...
      workerLogger.info(
        { jobId: job.id, jobName: job.job_name, workerId },
        "Job claimed and status updated to IN_PROGRESS",
      );
      ...
      workerLogger.info(
        { jobId: job.id, jobName: job.job_name, model },
      ...
      const jobContext = {
        jobId: job.id,
        jobDefinitionId:
          (job as any).job_definition_id ||
          job.parent_job_definition_id ||
          null,
      ...
      workerLogger.debug({ jobContext }, "Passing context to agent");
      ...
      workerLogger.retry(job.id, retryCount + 1, RETRY_CONFIG.maxRetries);
        ...
        try {
          ...
          if (current) {
            ...
            try {
              ...
              process.env.JINN_PROJECT_RUN_ID = (job.project_run_id ??
                "") as string;
              process.env.JINN_SOURCE_EVENT_ID = (job.source_event_id ??
                "") as string;
              process.env.JINN_PROJECT_DEFINITION_ID =
                (job.project_definition_id ?? "") as string;
              ...
              // restore env
              if (prevEnvBuzz.JINN_JOB_ID !== undefined)
                process.env.JINN_JOB_ID = prevEnvBuzz.JINN_JOB_ID;
              else delete process.env.JINN_JOB_ID;
              if (prevEnvBuzz.JINN_JOB_DEFINITION_ID !== undefined)
                process.env.JINN_JOB_DEFINITION_ID =
                  prevEnvBuzz.JINN_JOB_DEFINITION_ID;
              else delete process.env.JINN_JOB_DEFINITION_ID;
              if (prevEnvBuzz.JINN_JOB_NAME !== undefined)
                process.env.JINN_JOB_NAME = prevEnvBuzz.JINN_JOB_NAME;
              else delete process.env.JINN_JOB_NAME;
              if (prevEnvBuzz.JINN_PROJECT_RUN_ID !== undefined)
                process.env.JINN_PROJECT_RUN_ID =
                  prevEnvBuzz.JINN_PROJECT_RUN_ID;
              else delete process.env.JINN_PROJECT_RUN_ID;
              if (prevEnvBuzz.JINN_SOURCE_EVENT_ID !== undefined)
                process.env.JINN_SOURCE_EVENT_ID =
                  prevEnvBuzz.JINN_SOURCE_EVENT_ID;
              else delete process.env.JINN_SOURCE_EVENT_ID;
              if (prevEnvBuzz.JINN_PROJECT_DEFINITION_ID !== undefined)
                process.env.JINN_PROJECT_DEFINITION_ID =
                  prevEnvBuzz.JINN_PROJECT_DEFINITION_ID;
              else delete process.env.JINN_PROJECT_DEFINITION_ID;
            }
      ...
      if (!updateParseResult.success) {
        throw new Error(
          `Failed to update job to COMPLETED: ${updateParseResult.error}`,
        );
      }
      ...
      if (
        ...
        errorMessage = String(
          (err as any).error?.message || (err as any).error || "Unknown error",
        );
      } else {
      ...
      if (
        ...
        workerLogger.warn(
          `[RATE_LIMIT] Quota error detected, activating cooldown period (${RATE_LIMIT.cooldownAfterQuotaError}ms)`,
        );
        quotaErrorTime = Date.now();
      } else if (isInternalServerError(errorMessageLower)) {
        workerLogger.warn(
          `[RATE_LIMIT] 500/INTERNAL detected, activating short cooldown (${INTERNAL_ERROR_COOLDOWN_MS}ms)`,
        );
      ...
      if (
        ...
        workerLogger.error(
          { jobId: job.id, telemetry: (err as any).telemetry },
          `Job failed but captured error telemetry`,
        );
      } else {
        error = err;
      }
      ...
      try {
        ...
        const t =
          err && typeof err === "object" && "telemetry" in (err as any)
            ? (err as any).telemetry
            : undefined;
        const isTimeoutCategory = t?.errorType === "TIMEOUT";
        const po: string | undefined = t?.raw?.partialOutput;
        const hasTimeoutMarker =
          typeof po === "string" &&
          /\[PROCESS TERMINATED:\s*Process timeout/i.test(po);
        if (isTimeoutCategory || hasTimeoutMarker) {
          isRetryable = false;
          workerLogger.warn(
            { jobId: job.id },
            "Timeout detected; treating as non-retryable to persist partial output",
          );
        }
      } catch {}
      if (isRetryable && retryCount < RETRY_CONFIG.maxRetries) {
        ...
        workerLogger.warn(
          ...
          `Job failed with retryable error, retrying...`,
        );
        ...
        workerLogger.debug(
          ...
          `Enhanced prompt created for retry`,
        );
        workerLogger.debug(
        ...
        workerLogger.debug(
          {
            errorContextPreview: enhancedPrompt
              .substring(originalPrompt.length)
              .substring(0, 200),
          },
          `Error context added to retry prompt`,
        );
      ...
      workerLogger.info(
        { jobId: job.id, reason, isRetryable, retryCount },
        `Job not retrying`,
      );
...
async function processStakingOperations(
  stakingManager: OlasStakingManager,
): Promise<boolean> {
  const now = Date.now();

  // Check if it's time to run staking operations
  if (now - lastStakingTime < STAKING_INTERVAL_MS) {
    return false; // No staking operation needed
  }

  workerLogger.info("Executing periodic OLAS staking operations");

  try {
    await stakingManager.stakeOlas();
    lastStakingTime = now;
    workerLogger.info("OLAS staking operation completed successfully");
    return true;
  } catch (error) {
    workerLogger.error({ error }, "OLAS staking operation failed");
    // Don't update lastStakingTime on failure to allow retries
    return false;
  }
}
...
async function main() {
  workerLogger.info(
    { workerId, singleJobMode, targetJobId, debugMode, stopOnChief },
    "Worker starting up",
  );
  ...
  // Initialize OLAS staking manager
  const olasStakingManager = await StakingManagerFactory.createStakingManager();
  ...
  while (true) {
    try {
      ...
      // Process staking operations if manager is available
      let stakingProcessed = false;
      if (olasStakingManager) {
        stakingProcessed = await processStakingOperations(olasStakingManager);
      }

      if (!jobProcessed && !transactionProcessed && !stakingProcessed) {
        await DelayUtils.handleIdleDelay();
      } else if (!transactionProcessed && !stakingProcessed) {
        // If no transaction or staking was processed, add a small delay to prevent high CPU usage
        await DelayUtils.handlePartialWorkDelay();
      }
    } catch (error) {
      await DelayUtils.handleCriticalErrorDelay(error);
    }
  }
}
