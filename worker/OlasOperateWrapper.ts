...
export class OlasOperateWrapper {
  ...
  private async _startServer(): Promise<{ success: boolean; error?: string }> {
    ...
    try {
      ...
      const args = ['-m', 'operate.cli', 'daemon', `--port=${this.serverPort}`];
      const env: Record<string, string> = {};
      if (this.rpcUrl) {
      ...
      operateLogger.info({
        command: this.pythonBinary,
        args,
        cwd: this.middlewarePath,
        serverPort: this.serverPort
      }, "Starting operate server");

      this.serverProcess = spawn(this.pythonBinary, args, {
        cwd: this.middlewarePath,
        env: { ...process.env, ...env },
  ...
  private async _ensurePortAvailable(): Promise<void> {
    try {
      ...
      // If we get a response, there's a server running - try to shut it down
      if (response.ok) {
        operateLogger.info({ serverPort: this.serverPort }, "Found existing server on port, attempting graceful shutdown");
        try {
          await fetch(`http://${this.serverHost}:${this.serverPort}/shutdown`, {
            method: 'POST',
          ...
          operateLogger.debug({ shutdownError }, "Graceful shutdown failed, continuing");
        }
      }
    } catch (error) {
      // Port is likely available (connection refused/timeout)
