...
# Configuration
TIMEOUT=${TIMEOUT:-240}  # Default 4 minutes
TARGET="${1:---diff}"

echo "🔍 Running Discoverability Review (obj2: Code for the Next Agent) on: $TARGET"
echo "⏳ Analyzing for implicit and non-discoverable patterns..."
echo ""
...
claude -p "/review-obj2 $TARGET" --output-format json > "$TEMP_OUTPUT" 2>&1 &
CLAUDE_PID=$!
...
while kill -0 "$CLAUDE_PID" 2>/dev/null; do
  if [ $ELAPSED -ge $TIMEOUT ]; then
    ...
    echo ""
    echo "The discoverability review is taking longer than expected."
    echo "You can:"
...
if echo "$RESULT" | grep -q "Total violations found:"; then
  ...
  if [ "$VIOLATION_COUNT" = "0" ]; then
    echo -e "${GREEN}✅ No discoverability violations found!${NC}"
    echo -e "${BLUE}   Code is explicit and AI-friendly.${NC}"
    echo ""
    echo "$RESULT"
    exit 0
  else
    echo -e "${YELLOW}⚠️  Found $VIOLATION_COUNT discoverability violation(s)${NC}"
    echo -e "${YELLOW}   Code contains implicit or non-discoverable patterns.${NC}"
    echo ""
    echo "$RESULT"
    exit 1
  fi
  ...
  if echo "$RESULT" | grep -qi "error\|failed\|exception"; then
    ...
    echo -e "${GREEN}✅ Discoverability review completed (no violations detected)${NC}"
    echo ""
    echo "$RESULT"
    exit 0
  fi
