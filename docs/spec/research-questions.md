# Research Questions

Sources: Non‑Goals in [Technical Architecture - MVP](./mvp-spec) and research items in the [Roadmap](./roadmap).

## Items

- Venture token mechanics, Doppler/liquidity flows — Status: Pending
- veOLAS gauge specifics and distribution policy — Status: Pending
- Task Execution verification — Status: Pending
- Detailed executor internals and tool catalogs — Status: Pending
- Autonomous transaction development — Status: Pending
- Autonomous tool creation – Status: Pending
- Translate current centralised artefact sharing to decentralised environment — Status: Pending
- Translate current centralised memory creation and storage to decentralised environment — Status: Pending


