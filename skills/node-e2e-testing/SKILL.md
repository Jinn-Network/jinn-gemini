...
argument-hint: "[worker|wallet|all]"
---
...
Tenderly free tier allows ~5-10 write transactions per VNet. Full coverage requires two sessions:

| Argument | Sessions | What it tests |
|----------|----------|---------------|
| `worker` | 1 VNet | Setup → Dispatch → Worker execution |
| `wallet` | 1 VNet | Setup → Wallet info → Recovery (unstake + withdraw) |
| `all` | 2 VNets | `worker` session first, then fresh `wallet` session |
...
For `all`: complete the worker session (including cleanup), then start a fresh wallet session.
